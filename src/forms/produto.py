...
class ProdutoForm(FlaskForm):
    nome = StringField(label="Nome do Produto",
                       validators=[InputRequired(message="É obrigatorio definir o nome do produto"),
                                   Length(max=100,
    ...
    preco = DecimalField(label="Preço", places=2,
                         validators=[InputRequired(message="É obrigatorio definir o preço"),
                                     ...
                                     message="Os preços deve ser positivos")])
    ...
    ativo = BooleanField(label="Ativo?",
                         validators=[InputRequired(message="É preciso dizer se o produto esta o ou não ativo")])
    foto = FileField(label="Foto do produto",
                     validators=[FileAllowed(['jpg', 'png'], message="Apenas JPG ou PNG")])
